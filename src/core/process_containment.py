...
class ProcessContainment:
    ...
    def list_processes(
        self, project: str, zone: str, instance: str
    ) -> List[Dict[str, str]]:
    ...
    def kill_process(
        self, project: str, zone: str, instance: str, pid: str
    ) -> bool:
    ...
    def kill_by_name(
        self, project: str, zone: str, instance: str, process_name: str
    ) -> bool:
    ...
    def quarantine_file(
        self, project: str, zone: str, instance: str, file_path: str
    ) -> bool:
    ...
    def _run_command(
        self, project: str, zone: str, instance: str, command: str
    ) -> Optional[str]:
        ...
        try:
            ...
            serial_output = self._compute.instances().getSerialPortOutput(
                project=project,
                zone=zone,
                instance=instance,
                port=1,
            ...
            logger.error(
                f"GCE command execution error on {instance} "
                f"(project={project}, zone={zone}): {e}"
            )
            return None
