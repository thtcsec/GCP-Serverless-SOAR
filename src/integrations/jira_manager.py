...
class JiraManager:
        ...
        try:
            ...
            ticket_data = {
                "fields": {
                    ...
                    "summary": f"Security Incident - {severity_level} - {incident_data.get('isolation_result', {}).get('instance_id', 'Unknown Instance')}",  # noqa: E501
                    "description": description,
                    "issuetype": {"name": "Security Incident"},
                    "priority": {"name": jira_priority},
                    "labels": ["soar", "security-incident", severity_level.lower(), "automated"],
                    "customfield_10010": severity_level,  # Custom field for severity
                    "customfield_10011": priority,  # Custom field for priority
                    "customfield_10012": incident_data.get("original_finding", {}).get(
                        "account", "Unknown"
                    ),  # Account field
                }
            }
    ...
    def update_ticket_status(self, ticket_key, status, comment=None):
        ...
        try:
            ...
            if comment:
                transition_data["update"] = {"comment": [{"add": {"body": comment}}]}
...
---
*Generated by SOAR Platform at {datetime.now(UTC).strftime("%Y-%m-%d %H:%M:%S UTC")}*
"""
    ...
    def _add_watcher(self, ticket_key, watcher_email):
        ...
        try:
            watcher_data = {"self": watcher_email}
    ...
    def _make_jira_request(self, method, endpoint, data=None):
        ...
        try:
            ...
            response = requests.request(method=method, url=url, json=data, auth=auth, headers=headers, timeout=30)
...
def lambda_handler(event, context):
    ...
    try:
        ...
        logger.info("Jira operation completed successfully")
        return result
