...
_policy = get_policy_config()


class ScoringEngine:
    """Engine to calculate risk scores based on multi-source intelligence."""

    ANOMALY_BOOST = float(_policy["anomaly_boost"])
    ANOMALY_FLAG_THRESHOLD = float(_policy["anomaly_flag_threshold"])
    # Canonical multi-cloud policy bands (parity with AWS ScoringEngine / Nickel config)
    IGNORE_THRESHOLD = float(_policy["ignore_threshold"])
    AUTO_ISOLATE_THRESHOLD = float(_policy["auto_isolate_threshold"])
    ...
    def calculate_risk_score(
        ...
        Anomaly_boost applies when anomaly_score < anomaly_flag_threshold (from Nickel policy).
        """
