...
class SIEMForwarder:
    ...
    def forward_incident_data(self, incident_data):
        ...
        try:
            ...
            return {"forwarded": False, "error": str(e), "timestamp": datetime.now(UTC).isoformat()}
    ...
    def _transform_incident_for_siem(self, incident_data):
        ...
        siem_event = {
            ...
            "detection_time": severity.get("classification_timestamp", datetime.now(UTC).isoformat()),
    ...
    def _send_to_splunk(self, event):
        ...
        try:
            ...
            response = requests.post(url, json=splunk_event, headers=headers, timeout=30)
    ...
    def _send_to_chronicle(self, event):
        ...
        try:
            ...
            response = requests.post(url, json=chronicle_event, headers=headers, timeout=30)
    ...
    def _send_to_elastic(self, event):
        ...
        try:
            ...
            index_name = f"soar-incidents-{datetime.now(UTC).strftime('%Y.%m')}"
            ...
            ecs_event = {
                ...
                "agent": {"name": "soar-platform", "type": "soar", "version": "1.0"},
                "cloud": {
                    "account": {"id": event.get("account_id")},
                    "region": event.get("region"),
                },
                "host": {"hostname": event.get("instance_id"), "id": event.get("instance_id")},
                "soar": {
                    "incident": {
            ...
            if event.get("guardduty_finding"):
                ecs_event["aws"] = {"guardduty": {"finding": event["guardduty_finding"]}}

            response = requests.post(url, json=ecs_event, headers=headers, timeout=30)
...
def lambda_handler(event, context):
    ...
    try:
        ...
        logger.info("SIEM operation completed successfully")
        return result
