...
def process_storage_event(payload):
    ...
    if exfil_analysis["is_exfiltration"]:
        ...
        execute_exfiltration_response(bucket_name, principal_email, caller_ip, exfil_analysis, risk_data, intel_report)
...
def block_user_bucket_access(bucket_name, principal_email):
    ...
    try:
        ...
        if policy_changed:
            ...
            logger.warning(
                f"User {principal_email} had no direct bucket IAM bindings to remove. Assumed inherited from Project level."  # noqa: E501
            )
...
def create_forensic_snapshot(bucket_name, principal_email, analysis):
    ...
    try:
        ...
        forensic_blob = forensic_bucket.blob(
            f"storage-exfil/{bucket_name}/{principal_email}/{datetime.now(UTC).strftime('%Y%m%d%H%M%S')}.json"
        )

        forensic_blob.upload_from_string(json.dumps(forensic_data, indent=2), content_type="application/json")
...
def send_exfiltration_alert(
    ...
    try:
        ...
        try:
            ...
            desc = f"Exfiltration from {bucket_name} by {principal_email}\nAction: {action_status}\nIntel: {json.dumps(intel_report, indent=2, default=str)}"  # noqa: E501
